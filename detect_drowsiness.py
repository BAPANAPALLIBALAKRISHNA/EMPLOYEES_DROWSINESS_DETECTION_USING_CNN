...
while True:
    ...
    if len(faces) == 0:
        # Display alert message and play alarm sound
        cv2.putText(frame, "NO FACE DETECTED!", (100, height-20), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)
        if not alarm_on:
            alarm_on = True
    ...
    for (x, y, w, h) in faces:
        ...
        if status1 == 2 and status2 == 2:
            ...
            count += 1
            cv2.putText(frame, "EMPLOYER IS FEELING SLEEPY WITH Frame count: " + str(count), (2, 30), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 1)
            ...
            cv2.putText(frame, "EMPLOYER IS WORKING", (10, 30), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 255, 0), 1)
            count = 0
            alarm_on = False
